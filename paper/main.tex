...
\begin{abstract} % #TODO: rewrite
    ...
    This proposed Greedy-based method paves the way for more flexible design and more complex application scenarios, such as battery isolation, for RBS.
...
Taking the RBS shown in Fig.\ref{fig:stru-Visairo} as example, the batteries in the RBS can be connected not only in series when the switches $S_1$, $S_5$, $S_6$, $S_7$, $S_8$, $S_9$, and $S_{13}$ are closed, but also in parallel when $S_1$, $S_2$, $S_3$, $S_4$, $S_5$, $S_9$, $S_{10}$, $S_{11}$, $S_{12}$, and $S_{13}$ are closed.
Furthermore, when an unhealthy battery, for instance the orange one in Fig.\ref{fig:stru-Lawson}, exits in the RBS, it can be isolated by closing its three adjacent switches (i.e. $S_5$, $S_7$ and $S_8$) and opening the switch below that battery (i.e. $S_6$) to ensure the system still remains a reliable working mode.
...
With this proposed method, the MAC of RBSs with arbitrary structures can be calculated effectively, including application scenarios with isolated batteries.
...
These three structures will be employed to validate the effectiveness of this paper's proposed method for calculating the MAC. 
...
Utilizing Eq.\ref{eq:weight} and the Dijkstra algorithm, the $SP$s of the four batteries in the RBS structure of Fig.\ref{fig:study-stru-my} are shown in Figs.\ref{fig:sp1}-\ref{fig:sp4}, highlighted in red.
...
Furthermore, the significance of the method proposed in this paper is discussed from the perspective of next-generation RBS design.
...
This paper proposes a method for computing the MAC of an RBS pervasively and automatically based on its structure.
To identify the circuit within the RBS that enables the MAC, a greedy strategy combined with a directed graph model is developed.
The effectiveness of the proposed method is tested on a more complex RBS structure, which is based on two proposed structures.
The method remains effective for the application scenario of battery isolation in RBS and demonstrates that the new structure has the advantage of both output current flexibility and easy battery isolation.
Future research could focus on developing new performance metrics to evaluate the performance of the RBS using the currents and voltages obtained by the method, as well as modifying the equivalent model of the battery to allow for more accurate simulations of the RBS, including transient analysis.
