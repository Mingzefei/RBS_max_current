...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{figures/stru-V-origin.png}
        \caption{}
        \label{fig:stru-Visairo}
    \end{subfigure}
    \hspace{0.05\textwidth}
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{figures/stru-V-serial.png}
        \caption{}
        \label{fig:stru-Visairo-serial}
    \end{subfigure}
    \\
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{figures/stru-V-parallel.png}
        \caption{}
        \label{fig:stru-Visairo-parallel}
    \end{subfigure}
    \hspace{0.05\textwidth}
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{figures/stru-V-isolate.png}
        \caption{}
...
\added{The main contributions of this paper can be summarized as follows:}
\begin{itemize}
  \item \added{An efficient method is proposed to determine the MAC of RBSs with arbitrary structures, including scenarios with isolated batteries.}
  \item \added{The greedy algorithm is applied to solve the MAC problem, the computational complexity of which is greatly reduced compared with the brute-force algorithm.}
  \item \added{An improved directed graph model is introduced; it considers the voltage, the internal resistance, the MAC of the battery, and the external load to analyze the current of the RBS.}
\end{itemize}
...
First, a directed graph model is established for subsequent \replaced{computations.}{computing,} \replaced{The model}{, which} not only contains the connected relationships between batteries and switches but also retains the performance parameters of the batteries.
...
The shortest paths (\replaced{$SP$}{SP}s, where additional batteries and switches on the path are penalized as distance) for the batteries are then obtained  by using the Dijkstra algorithm to connect the batteries in the RBS in parallel.
Finally, a greedy algorithm is used to organize the switches, allowing the batteries to connect via their \replaced{$SP$}{SP}s while satisfying the constraints, resulting in the MAC of the RBS.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.31\textwidth}
        \includegraphics[width=\textwidth]{figures/direct-graph-he.png}
        \caption{}
        \label{fig:direct-graph-he}
    \end{subfigure}
    \hspace{0.02\textwidth}
    \begin{subfigure}[b]{0.23\textwidth}
        \includegraphics[width=\textwidth]{figures/direct-graph-xu.png}
        \caption{}
        \label{fig:direct-graph-xu}
    \end{subfigure}
    \hspace{0.02\textwidth}
    \begin{subfigure}[b]{0.24\textwidth}
        \includegraphics[width=\textwidth]{figures/direct-graph-my.png}
        \caption{}
        \label{fig:direct-graph-my}
    \end{subfigure}
    \\
    \begin{subfigure}[b]{0.8\textwidth}
        \includegraphics[width=\textwidth]{figures/battery_simple.png}
        \caption{}
...
The model captures all potential system configurations and offers a direct metric for configuration flexibility, but it does not specify the physical implementation of the connectivity between batteries, meaning that one graph might \replaced{correspond to}{have had} multiple RBS structures.
We previously proposed a directed graph model that \replaced{differs completely from}{in contrast to} He's model by using nodes to represent the connections between batteries and switches and directed edges to represent batteries and switches (Fig. \ref{fig:direct-graph-xu}), allowing for a one-to-one correspondence between the RBS structure and the directed graph model. 
This model accurately and comprehensively represents the RBS topological structure but cannot be used for quantitative MAC calculations because it does not consider  \replaced{the voltage, internal resistance, and MAC of the battery}{battery and switch performance parameters}. 
To address this issue, \replaced{we improve our previous model by}{improved directed graph model is used here based on our original model,} adding electromotive force and resistance attributes on the edges based \replaced{on its}{to} equivalent circuits\deleted{(Fig. \ref{fig:direct-graph-my})}.
...
\added{Fig. \ref{fig:direct-graph-my} shows the improved directed graph model used in this paper.}
...
\added{For instance,} \replaced{a}{A}s shown in Fig. \ref{fig:battery_simple}, the battery in the RBS can be represented as a black-box circuit consisting of two resistors $r_1$ and $r_2$ and a capacitor  $C$, known as the Thevenin model \cite{hongwenheStateofChargeEstimationLithiumIon2011,mousavig.VariousBatteryModels2014}.
...
\replaced{For a given RBS, determining its MAC}{Based on the definition of MAC, determining the MAC of RBS} involves maximizing the RBS output current while ensuring that the sum of the currents of all batteries does not exceed the batteries' MAC. 
This subsection establishes the constraints and objective function to determine the RBS's MAC  through circuit analysis based on \replaced{the directed graph model provided in the previous section}{the previously constructed directed graph model}.


First, the topology in the directed graph model is represented in matrix form $\bm{A}$, known as the incidence matrix and \replaced{defined}{to facilitate circuit analysis.
The specific definition of the incidence matrix is shown} as follows:
...
The matrix $\bm{Y}$ in \added{Eq.} (\ref{eq:generalized_branches}) is the admittance matrix of the circuit and is defined as the inverse of the impedance matrix.
\replaced{The}{That is the} elements \replaced{on}{of} the diagonal \added{of} matrix $\bm{Y}$ are equal to the reciprocal of \added{the resistance, which is} the second attribute of the corresponding edges in the directed graph. The off-diagonal elements of \added{$\bm{Y}$} are zero.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.2\textwidth}
        \includegraphics[width=\textwidth]{figures/stru-L-origin.png}
        \caption{}
    ...
    \hspace{0.02\textwidth}
    \begin{subfigure}[b]{0.31\textwidth}
        \includegraphics[width=\textwidth]{figures/stru-my-origin.png}
        \caption{}
...
The primary goal of Visairo's structure (Fig. \ref{fig:study-stru-Visairo}) is to dynamically adjust the RBS output \added{power}. However, the isolation of unhealthy batteries is not sufficiently addressed \added{in their work}. 
...
Lawson et al. \replaced{designed the RBS structure shown in Fig. \ref{fig:study-stru-Lawson} to isolate batteries}{conducted research on battery isolation in RBS and specifically designed the structure shown in Fig. \ref{fig:study-stru-Lawson}}. 
Although this structure easily isolates batteries, it cannot dynamically adjust the output current of the RBS. 
Based on the structures of Visairo and Lawson, this paper proposes the structure shown in Fig. \ref{fig:study-stru-my}\deleted{, which combines the advantages of both}.
...
Finally, the calculated MACs of the structure in Fig. \ref{fig:study-stru-my} are listed in Table \ref{tab:study-results-my} and Fig. \ref{fig:study-results-my}, as obtained by the greedy algorithm 1. % \ref{alg:greedy}.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.28\textwidth}
        \includegraphics[width=\textwidth]{figures/ef-topo.png}
        \caption{}
        \label{fig:study-dirgraph-my}
    \end{subfigure}
    \hspace{0.05\textwidth}
    \begin{subfigure}[b]{0.28\textwidth}
        \includegraphics[width=\textwidth]{figures/ef-sp1.png}
        \caption{}
        \label{fig:sp1}
    \end{subfigure}
    \hspace{0.05\textwidth}
    \begin{subfigure}[b]{0.28\textwidth}
        \includegraphics[width=\textwidth]{figures/ef-sp2.png}
        \caption{}
        \label{fig:sp2}
    \end{subfigure}
    \\
    \begin{subfigure}[b]{0.28\textwidth}
        \includegraphics[width=\textwidth]{figures/ef-sp3.png}
        \caption{}
        \label{fig:sp3}
    \end{subfigure}
    \hspace{0.05\textwidth}
    \begin{subfigure}[b]{0.28\textwidth}
        \includegraphics[width=\textwidth]{figures/ef-sp4.png}
        \caption{}
        \label{fig:sp4}
    \end{subfigure}
    \hspace{0.05\textwidth}
    \begin{subfigure}[b]{0.28\textwidth}
        \includegraphics[width=\textwidth]{figures/ef-mac.png}
        \caption{}
...
\begin{table}[htbp]
  ...
  \begin{figure}[htbp]
      \centering
      \begin{subfigure}[b]{0.31\textwidth}
          \includegraphics[width=\textwidth]{figures/my-isolated-1.png}
          \caption{}
          \label{fig:my-isolated-1}
      \end{subfigure}
      \hspace{0.02\textwidth}
      \begin{subfigure}[b]{0.31\textwidth}
          \includegraphics[width=\textwidth]{figures/my-isolated-2b.png}
          \caption{}
          \label{fig:my-isolated-2b}
      \end{subfigure}
      \\
      \begin{subfigure}[b]{0.31\textwidth}
          \includegraphics[width=\textwidth]{figures/my-isolated-2w.png}
          \caption{}
          \label{fig:my-isolated-2w}
      \end{subfigure}
      \hspace{0.02\textwidth}
      \begin{subfigure}[b]{0.31\textwidth}
          \includegraphics[width=\textwidth]{figures/my-isolated-3.png}
          \caption{}
...
\replaced{Consider first the results shown in Fig. \ref{fig:all-results-my} and Table \ref{tab:study-results-my}}{In this subsection, we firstly discuss the correctness of the results presented in Fig. \ref{fig:all-results-my} and Table \ref{tab:study-results-my}}.
...
\added{The MAC obtained by the brute-force algorithm is identical to the one obtained by the greedy algorithm.}
...
According to  Appendix 1, % \ref{alg:greedy},
the RBS can reconfigure $C^{N_{\text{set}}}_{N_b}$ structures by selecting $N_{\text{set}}$ batteries from $N_b$ batteries, which gives $\sum^{N_b}_{N_{\text{set}}=1}C^{N_{\text{set}}}_{N_b}/N_b \approx 2^{N_b}/N_b$ on average.
...
Note that \deleted{when solving for MAC,} $\eta$ is used as the objective function instead of $I_o$ \added{in solving for the MAC}. 
...
\replaced{However, when}{If} $I_o$ \replaced{is}{were} used as the objective function, even for the same RBS structure, the MAC \replaced{solution}{result} and corresponding switch states could change due to different external electrical appliances.
This would increase the difficulty and uncertainty of designing the RBS structure. 
\replaced{To eliminate this problem, the ratio $\eta=I_o/\max\bm{I}_b$ is adopted as the objective function in our research.}{In contrast, by using $\eta$ as the objective function, which is defined as the ratio of $I_o$ and $\max\bm{I}_b$, the influence of these factors on the results can be eliminated.}
...
The method proposed in this paper facilitates the design of \deleted{next-generation} RBSs in the following ways:
...
\replaced{In contrast}{Owing to the advantages of pervasiveness and automation}, the proposed method calculates the MAC of arbitrary RBS structures, \replaced{notably the complex and flexible RBS structures}{which helps to address the aforementioned issues and paves the way for more complex and flexible RBS structure design}.


To illustrate this point, the MACs of \deleted{the} three RBS structures mentioned above are calculated after \replaced{isolating one or more}{the} of the batteries, as shown in Table \ref{tab:isolated_mac}. 
Specifically, for the structure presented in Fig. \ref{fig:study-stru-my}, the corresponding circuit states for the MACs when isolating \replaced{one to three}{different numbers of} batteries are depicted in Figs. \ref{fig:my-isolated-1}--\ref{fig:my-isolated-3}. 
...
The results \added{in Figs. \ref{fig:my-isolated-1}--\ref{fig:my-isolated-3}} show that the proposed method provides reasonable outcomes for isolating any number of batteries in any position.


Furthermore, the \deleted{performance of} output current for the three RBSs \replaced{with isolated}{when isolating} batteries is also shown in Table \ref{tab:isolated_mac}. 
For the structure proposed by Lawson et al., the MAC \replaced{is independent of the number of isolated batteries}{remains the same as that without isolated battery cells, i.e., $\eta=1$, when the number of isolated battery cells increases, until all the cells in the RBS are isolated}.
\added{However,} for Visairo's structure, the MAC decreases \replaced{upon increasing the number of isolated batteries}{as the number of isolated battery cells increases, until $\eta=0$}. 
\replaced{Nevertheless}{In contrast}, the MAC of the structure proposed in this work falls between \replaced{the}{the} MACs of these two structures. 
This result indicates that the structure proposed in this paper has a larger MAC than Lawson's for the same number of batteries \replaced{and has}{which means} a wider range of regulation of the output current. 
...
This paper proposes a pervasive and automated method to efficiently compute the MAC of \replaced{an}{the}  RBS.
...
%     {
%         $N_{\text{set}} \leftarrow \text{number of setected $SP$s calculated by dichotomy}$\;
%         $C_b    \leftarrow \text{set of all combinations of $N_{\text{set}} $~batteries from $N_b$}$\;
%         \For{$c_b \in C_b$}{
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/algorithm.jpg}
\end{figure}
