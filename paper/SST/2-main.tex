...
During RBS operation, the maximum allowable current (MAC) of the system that ensures that each battery's current remains within a safe range is a critical indicator to guide the system's reconfiguration, ensuring its safety and reliability. % TODO ensure
...
These problems have become significant technical barriers in the development of new-generation space vehicles and need to be addressed \cite{pomboHybridPowerSystem2021}. % TODO 落到电池可靠性上，提一句例如，space vehicle
...
Moreover, the method provides an improved direct graph model that considers the voltage, the internal resistance, the MAC of the battery, and the external load. The model obtains the accurate current of the RBS under a specific circuit topology. % TODO 和句话风格（主语）一致；和下面区分
...
% TODO 先写研究成果，数值方面的结论（提升的具体数值，具体的条件下）
