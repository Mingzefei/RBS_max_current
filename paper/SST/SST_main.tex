...
\added{
By introducing the shortest path of the battery, the greedy algorithm reconfigures the structure that can output MAC more efficiently, compared to the complete brute force algorithm method.
The directed graph model is based on the equivalent circuit and provides a specific method for calculating the MAC of a given structure.
This method has been validated on two published 4-battery-RBSs and one with more complex structure.
It obtaining the same results as the brute force algorithm method, while achieves significantly improved computational efficiency (3000 to 75000 times, primarily depending on the number of switches).
...
Compared to the brute force algorithm, the efficiency of the method based on the greedy algorithm has been improved by 3000 to 75000 times.
...
(1) The SPs guide the RBS to reconfigure reasonable structures, rather than blindly going through all possible structures. This reduces the factor in complexity from $2^{N_s}$ to $2^{N_b}$, which is the main reason for the improvement in efficiency;
