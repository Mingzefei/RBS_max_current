...
\added{
% 目前，许多研究人员利用 RBS的灵活地可重构特点，设计多种结构，以实现并改善系统的 performance
% Ci【139】等人提出的结构能够动态调整电池放电倍率以充分挖掘各电池的可用容量；
% Jan【149，150】等人针对电动汽车充电时不断变化的电压需求，提出了一种能够动态调整电池串联数的结构。
% Visairo 【140】等人的结构可以根据负载条件改变系统的输出电压，从而减少供电过程中电压调节器的功率损耗以提升能量利用率；
% 同样处于提升系统能量效率的目的，Lawson等人\cite{lawsonSoftwareConfigurableBattery2012}和He等人【148】也提出过更为简化的结构。Kim等人【143】则通过向结构引入多端口，提升了系统对电池故障的恢复能力。
Currently, many researchers utilize the flexibility and reconfigurability of RBS to design various structures in order to meet application requirements and improve system performances. 
Ci et al. \cite{ci2007novel} proposed a RBS structure that can dynamically adjust the batteries discharge rate to fully exploit the available capacity of each battery. 
...
% 这些结构的提出很大程度上依赖研究人员的经验，并且缺少严谨的结构分析方法和客观全面的评价指标。
% 这可能会导致结构设计不够优化，无法充分发挥RBS的性能优势；甚至存在潜在的安全隐患，无法让RBS投入实际应用。
...
% This research goes through and analyzes all paths between the positive and negative terminals of each battery in the RBS, and identifies paths that only contain switches as short-circuit paths to form a short-circuit path table for pre-check and before system reconfiguration.
% Ham【162】等人则针对一种特定的RBS结构，推导出 maximum switch current during battery system reeconfiguration 的解析表达式。
% 该工作可用于指导RBS的开关选型，为RBS的硬件设计提供支撑。
...
\added{
% 当前，对于RBS许用电流的分析仍是空白。
% 电池具有合适的工作电流范围，过大的电流会导致电池的性能下降和衰退，甚至损坏电池。【文献】
% 通过有限数量的电池组成的RBS也同样存在系统输出电流的上限，即 maximum allowable current (MAC)。
% 超过该阈值会导系统内部部分电池的电流过载。
% 因此，需要一种能够确定系统 MAC 的 RBS结构的电流分析方法和求解算法
There is still a blank of analysis the allowable current of RBS.
The battery has an appropriate range of operating currents, and excessive current can lead to a decrease and degradation in battery performance, or even damage\cite{lotfi2017li}.
For RBS composed of finite batteries, there is also an upper limit on the system's output current, known as the maximum allowable current (MAC). 
Exceeding this threshold can result in current overload on certain batteries of the system. 
Therefore, there is a requirement for analysis method on RBS current and solving algorithm that can determine the system's MAC.
The maximum flow problem (MFP) in graph theory has a similar formulation to the RBS MAC determination.
...
Meanwhile, the Kirchhoff's laws that describe the current in circuit, and the function of switches to control the circuit on-off should be appropriately translated into constraints in the MFP.
This strongly increases the complexity and makes it a special maximum flow problem that never been studied before.
Despite mature algorithms for the MFP, such as the Edmonds-Karp algorithm \cite{10.1145/321694.321699} and Dinic's blocking flow algorithm \cite{doi:10.1137/0204043}, these algorithms cannot be directly applied to solve the MAC due to the speciality of the problem. 
Moreover, these MFP algorithms need to comprehensively compute all possible reconfigured structures to obtain the MAC, which means highly inefficience and not suitable for large-scale RBS.
}
...
\added{
To effectively solve the switches states of MAC while strongly decreasing the unnecessary reconfigured structures to compute, and still robust in the scenarios with battery isolation, the greedy algorithm is adopted and adapted to solve the MAC problem in this paper.
Meanwhile, an improved direct graph model, considering the voltage, internal resistance, and maximum allowable current of the battery, and the external load, is porposed and emploied to obtain the accuracy value of MAC.
The correctness and effectiveness of the method in this paper are verified by two published 4-battery-RBSs and one with more complex structure.
}
...
\begin{itemize}
  ...
  \item \added{Presenting a improved directed graph model to analyse the current of RBS, which considers the voltage, internal resistance, and maximum allowable current of the battery, and the external load.}
  \item \added{Proposing a new RBS structure and comparing it with two proposed RBS structures in the scenario with randomly isolated batteries.}
\end{itemize}
...
\added{The results of MAC obtained by going through all reconfigured structures is identical to the one by the greedy algorithm. 
...
Therefore, the time complexity of the proposed algorithm is analyzed and compared with the algorithm that goes through all reconfigured structures to demonstrate the advantage of this algorithm.
...
Therefore, it is reasonable to believe that as the size of the RBS increases, especially the number of the swithes, the greedy algorithm will have an advantage over the algorithm that goes through all reconfigured structures.
This can be comfirmed from the number of structures required for MAC determination in the previous subsection. 
...
\newblock 68(12):12353--12363, 2021.
...
\newblock {\em IEEE Transactions on Industrial Electronics}, 69(6):5931--5941,
  2021.
