...
A \replaced{automatical and universal}{universal and automatical} method for calculating RBS MAC is \deleted{urgently} needed for practical applications. 
...
\deleted{In this subsection, we firstly discuss the correctness of the results presented in Figure \ref{fig:all-results-my} and Table \ref{tab:study-results-my}.} 
...
The method is  implemented by a greedy algorithm combined with a directed graph model, \replaced{and}{whose effectiveness is tested on a novel and complex RBS structure.
The method} remains effective for the application scenario of \replaced{the RBS system with a complex structure and isolated batteries}{RBS battery isolation and demonstrates that the novel structure has the advantage on flexible output current and convenient battery isolation}.
