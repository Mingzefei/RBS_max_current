...
\title{Determining Maximum Allowable Current of an RBS using a Directed Graph Model and Greedy Algorithm}
...
This paper proposes a method for calculating the MAC of an arbitrary RBS using a greedy algorithm in conjunction with a directed graph model of the RBS.
Using the shortest path of the battery, the greedy algorithm transforms the exhaustion of the switch states in the brute-force algorithm or variable search without utilizing structures in the heuristic algorithms in the combination of the shortest paths.
...
The results are the same as those from the brute-force algorithm, but the proposed method significantly improves the computational efficiency, being theoretically $N_s2^{N_s-N_b}\log_{10}N_b$ times faster than the brute-force algorithm for an RBS with $N_b$ batteries and $N_s$ switches.
...
Accordingly, this proposed method can facilitate the full utilization of the RBS's current output potential, guide the design and optimization of the RBS structure, and assist in evaluating the risk of current overload in practical applications.
Its further application potentially focuses on the future explorations in battery energy management of the space power system in a spacecraft, to open new avenues for enhancing its efficiency and reliability.
