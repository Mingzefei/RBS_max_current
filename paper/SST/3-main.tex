...
%       2.1.4 4-battery 不同结构 isolated battery 的计算结果：1组fig（不同电池数的开关闭合方案）
%   2.3 discussion:
...
%<*reviewer1-comment2>
The early research on RBSs focused on the design of the structure with different flexibility and reconfigurability to meet application requirements.
...
Those complex structures between batteries and switches give RBSs flexibility but also creates the challenge in the design of the hardware. 
The variation of current occurs during the system reconfigurating.
...
To develop the effective control strategies of RBSs to achieve the optimal performance, such as better stability \cite{kacetlDesignAnalysisModular2023b} and efficiency \cite{yangAdaptiveControlFramework2022}, is another hot research topic.
...
Liu et al. \cite{liuFlexiblePathPlanningbased2023a} also proposed a maximum capacity utilization scheme based on a path planning algorithm to improve the battery consistency in the system.
...
Furthermore, AI-based management is also used in the system \cite{liuLongLifetimeBattery2022a}.
...
Unfortunately, few studies have been proposed to determine the RBS MAC.
% scheduling tasks onto dynamically reconfigurable hardware 的方法可以考虑资源有限和任务依存的问题，但是难以处理 RBS 的结构特征和电路方程。
% 从 RBS 结构分析的角度分析，求解 MAC 的问题转化为在 RBS 的所有可能重构出的结构中寻找最大输出电流。
% 但是这可能会是一个 NP-hard 问题 pinterReviewControlAlgorithms2021a 
...
% 启发式算法，如SA和GA，也可以用于求解该问题，但是不能完全保证获得最优解，而且收敛时间较长。z
% 这些方法都不能满足设计和控制时，对 MAC 的快速求解需求。
%</reviewer1-comment2>
