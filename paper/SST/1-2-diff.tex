...
%DIF DEL 1-main.tex   Sat Sep 16 00:15:02 2023
%DIF ADD 2-main.tex   Sat Sep 16 00:13:41 2023
% 【version: SST-1.4】 %DIF > 
%DIF -------
...
% 【done】  %DIF > 
% =====0909 %DIF > 
% 按 Q 修改意见修改 0901 【优先】 %DIF > 
% =====0910 %DIF > 
% 1. 摘要修改，量化我们的成果：explain the most important achievements of the proposed method quantitatively %DIF > 
% 2. 介绍修改，相关文献： %DIF > 
%   2.1 RBS 结构分析相关的 %DIF > 
%   2.2 网络流（最大流）相关（但因为问题不同，无法使用，提一句穷举） %DIF > 
% 3. 介绍修改，逐点列出我们的贡献：state research contribution and achievements as bullet points at the end of the Introduction section %DIF > 
% 4. 讨论修改，增加算法复杂度，以及和穷举法的比较（该领域尚为空白，没有sota的算法） %DIF > 
% 5. 讨论修改，增加算法的 limitations（根据复杂度，大规模不好用） %DIF > 
% 6. 讨论修改，增加结果的准确性介绍（和穷举结果比较）：explain about the accuracy, sufficiency and reliability of their results? How do they verify and validate the results %DIF > 
% 7. 修改算法的第15步 %DIF > 
% 8. 重写总结 %DIF > 
% 9. 添加文献 %DIF > 
% =====0911 %DIF > 
% 1. introduction 整体逻辑不流畅，跳跃过大 %DIF > 
% 2. introduction 文献 %DIF > 
% 2. conclusion 结尾，可以加上一两句说说这个方法的应用价值之类的，吹一吹 %DIF > 
% 3. abstract %DIF > 
%   3.1 删去 75000 倍的效率； %DIF > 
%   3.2 凝练一点，就是说这个方法把遍历法中的枚举开关状态组合，转换为了对最短路径的组合； %DIF > 
%   3.3 算法复杂度、效率提升倍数=2^开关数/2^电池数 %DIF > 
%   3.4 调整 discussion 的相关内容 %DIF > 
% 4. 删去 最大流 %DIF > 
% 5. 按 Q 的意见修改 %DIF > 
% =====0913 %DIF > 
% 1. 按师兄的修改意见修改（0912晚） %DIF > 
% 2. 调整文献顺序，改用 biber %DIF > 
% =====0915 %DIF > 
% my structure, my method 的计算结构数为 11，而不是 7 %DIF > 
 %DIF > 
% 【doing】 %DIF > 
 %DIF > 
% 【todo】 %DIF > 
% ==18号润色完再次核对，21或22号提交。 %DIF > 
 %DIF > 
%DIF -------
...
% \added{xxx} %DIF > 
% \deleted{xxxx} %DIF > 
% \replaced{a}{b} 原文的b替换成a %DIF > 
% \usepackage{easyReview} % texstudio 部分 %DIF > 
 %DIF > 
%DIF -------
%%%%%% Bibliography %%%%%%
...
During the operation of the RBS, the Maximum Allowable Current (MAC) of system that ensures each battery\DIFdelbegin \DIFdel{’}\DIFdelend \DIFaddbegin \DIFadd{'}\DIFaddend s current remains within a safe range, is a critical indicator to guide the system\DIFdelbegin \DIFdel{’}\DIFdelend \DIFaddbegin \DIFadd{'}\DIFaddend s reconfiguring control, ensuring its safety and reliability. 
...
The results show that }\DIFdelend \DIFaddbegin \DIFadd{equivalent circuit, provides a specific method for calculating the MAC of a given structure.
...
Moreover, if this worst battery fails \DIFdelbegin \DIFdel{during }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend operation, it can exacerbate the degradation of other batteries with a high possibility, leading to reliability and safety issues \cite{yangUnbalancedDischargingAging2016,fengPropagationMechanismsDiagnosis2019,jeevarajanBatterySafetyQualifications2012}.
These problems have become significant technical barriers in the development of new-generation space vehicles \DIFdelbegin \DIFdel{and urgently need to be addressed }\DIFdelend \DIFaddbegin \DIFadd{unfortunately }\DIFaddend \cite{pomboHybridPowerSystem2021}.
...
The ability of switching circuit helps to isolate unhealthy batteries, \DIFdelbegin \DIFdel{and thereby improve }\DIFdelend \DIFaddbegin \DIFadd{thereby improving }\DIFaddend the safety and reliability of the battery system.
\DIFdelbegin \DIFdel{Figure \ref{fig:stru-Visairo} shows }\DIFdelend \DIFaddbegin \DIFadd{In order to illustrate the working principle of RBS, }\DIFaddend a typical RBS structure developed by Visairo \cite{visairoReconfigurableBatteryPack2008}\DIFdelbegin \DIFdel{for dynamically adjusting the output voltage and current}\DIFdelend \DIFaddbegin \DIFadd{(Figure \ref{fig:stru-Visairo}) is taken as an example to show the reconfiguration process}\DIFaddend .
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{stru-V-origin}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{stru-V-origin.png}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{stru-V-serial}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{stru-V-serial.png}
    ...
    \\
    \begin{subfigure}[b]{0.45\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{stru-V-parallel}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{stru-V-parallel.png}
    ...
    \hspace{0.05\textwidth}
    \begin{subfigure}[b]{0.45\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{stru-V-isolate}
%DIFDELCMD <         %%%
...
\DIFdelbegin \DIFdel{The complex connection }\DIFdelend \DIFaddbegin \DIFadd{Recently, various types of RBS with different flexibility and reconfigurability have been designed to meet application requirements. 
For example, Ci et al. \mbox{%DIFAUXCMD
\cite{ci2007novel} }\hskip0pt%DIFAUXCMD
proposed a RBS structure that can dynamically adjust the batteries discharge rate to fully exploit the available capacity of each battery. 
Jan's \mbox{%DIFAUXCMD
\cite{9209774,engelhardt2021double} }\hskip0pt%DIFAUXCMD
structures had the ability to reconfigure structures with variant batteries in series to reach the voltage requirements during electric vehicle charging, which constantly change.
The structure proposed by Visairo et al.\mbox{%DIFAUXCMD
\cite{visairoReconfigurableBatteryPack2008}}\hskip0pt%DIFAUXCMD
, as shown in Figure \ref{fig:stru-Visairo}, can change the system's output voltage based on load conditions, thereby reducing the power loss of the voltage regulator during the power supply process and improving energy utilization efficiency. 
For the same purpose to enhance the energy efficiency of the system, Lawson et al. \mbox{%DIFAUXCMD
...
also proposed simplified structures which has less switches than Visairo's design.
Kim et al. \mbox{%DIFAUXCMD
\cite{kim2009dynamic} }\hskip0pt%DIFAUXCMD
improved the system's ability to recover from battery failures by introducing multiple ports to the structure. 
The complex }\DIFaddend structure between batteries and switches \DIFdelbegin \DIFdel{in the RBS provides flexibility but also introduces }\DIFdelend \DIFaddbegin \DIFadd{enables the flexibility of RBS, but also brings the }\DIFaddend challenges in design and \DIFdelbegin \DIFdel{operational control . 
...
The Maximum Allowable Current }\DIFdelend \DIFaddbegin \DIFadd{control of system. 
...
proposed a systematic approach based on the sneak circuit theory to fundamentally avoid the short-circuit problem of reconfigurable battery systems. 
...
\DIFadd{In spite of maximum switch current mentioned above, the maximum allowable current }\DIFaddend (MAC), \DIFdelbegin \DIFdel{an RBS performance indicator, can guide designers in addressing this issue. 
...
Therefore, it provides a benchmark for RBS output }\DIFdelend current \DIFdelbegin \DIFdel{, protecting individual batteries and identifying overall system output limits during operation.
Despite its importance, no method currently exists for automatically evaluating MAC for RBSs .
In particular, when one or more random cells are isolated, there is still no method to determine }\DIFdelend \DIFaddbegin \DIFadd{allowed by system under the constraints of the battery cell, is another critical indictor of RBS that need to be evaluated during the design or control process of system. 
...
\DIFadd{To solve this issue, a new method to effectively evaluate }\DIFaddend the MAC of \DIFdelbegin \DIFdel{the remaining RBS in time to assist the system in adjusting the control strategy timely. 
A universal and automatical method for calculating RBS MAC is urgently needed for practical applications.
In this study, a directed graph modeland greedy algorithm are employed to determine the MAC of RBS and the corresponding control strategy, effectively calculating the MAC for }\DIFdelend \DIFaddbegin \DIFadd{RBS is proposed in this paper. 
...
The main contributions of the paper can be summarized as follows:
  ...
  \item \DIFadd{Proposing a effective method to determine the MAC of }\DIFaddend RBSs with arbitrary structures, including scenarios with isolated batteries.
  \DIFaddbegin \item \DIFadd{Application of the greedy algorithm to solve the MAC problem, whose computational complexity is greatly reduced compared with the brute force algorithm.
  }\item \DIFadd{introducing an improved directed graph model, considering the voltage, internal resistance, and maximum allowable current of the battery, and the external load, to achieve the current analysis of the RBS.
}\end{itemize}
\DIFaddend 
...
The calculation results\DIFaddbegin \DIFadd{, the algorithm's computational complexity }\DIFaddend and scenarios such as batteries isolation also are discussed. 
...
Firstly, a directed graph model is established for subsequent \DIFdelbegin \DIFdel{computing, which }\DIFdelend \DIFaddbegin \DIFadd{computations. The model }\DIFaddend not only contains the connected relationships between batteries and switches, but also retains the performance parameters of the batteries.
...
Then, the shortest paths (\DIFdelbegin \DIFdel{SPs}\DIFdelend \DIFaddbegin \DIFadd{$SP$s}\DIFaddend , where additional batteries and switches on the path are penalized as distance) for the batteries are obtained using the Dijkstra algorithm to guide the batteries in the RBS connect in parallel.
Finally, a greedy algorithm is employed to organize the switches, allowing the batteries to connect via their \DIFdelbegin \DIFdel{SPs }\DIFdelend \DIFaddbegin \DIFadd{$SP$s }\DIFaddend while satisfying the constraints, resulting in the MAC of the RBS.

\begin{figure}[htbp]
    \centering
    \begin{subfigure}[b]{0.8\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{main}
%DIFDELCMD <         %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
}
...
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{main.png}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.31\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{direct-graph-he}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{direct-graph-he.png}
        \DIFaddendFL \caption{}
        \label{fig:direct-graph-he}
    \end{subfigure}
    \hspace{0.02\textwidth}
    \begin{subfigure}[b]{0.23\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{direct-graph-xu}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{direct-graph-xu.png}
        \DIFaddendFL \caption{}
        \label{fig:direct-graph-xu}
    \end{subfigure}
    \hspace{0.02\textwidth}
    \begin{subfigure}[b]{0.24\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{direct-graph-my}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{direct-graph-my.png}
        \DIFaddendFL \caption{}
        \label{fig:direct-graph-my}
    \end{subfigure}
    \\
    \begin{subfigure}[b]{0.8\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{battery_simple}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{battery_simple.png}
        \DIFaddendFL \caption{}
...
The model effectively captured all potential system configurations and offered a direct metric for configuration flexibility, but it did not specify the physical implementation of the connectivity between batteries, meaning one graph might \DIFdelbegin \DIFdel{have had }\DIFdelend \DIFaddbegin \DIFadd{correspond to }\DIFaddend multiple RBS structures.
We previously proposed a novel directed graph model that, \DIFdelbegin \DIFdel{in contrast to }\DIFdelend \DIFaddbegin \DIFadd{completely different from }\DIFaddend He's model, used nodes to represent the connections between batteries and switches, and directed edges to represent batteries and switches (Figure \ref{fig:direct-graph-xu}), allowing for a one-to-one correspondence between the RBS structure and the directed graph model. 
This model was able to accurately and comprehensively represent the RBS topological structure but could not be used for quantitative MAC calculations due to the lack of consideration \DIFdelbegin \DIFdel{for battery and switch performance parameters}\DIFdelend \DIFaddbegin \DIFadd{on the voltage, internal resistance and maximum allowable current of the battery}\DIFaddend . 
To address this, an \DIFdelbegin \DIFdel{improved directed graph model is used here based on our original model , }\DIFdelend \DIFaddbegin \DIFadd{improvement on our previous model is conducted by }\DIFaddend adding electromotive force and resistance attributes on the edges based \DIFdelbegin \DIFdel{to equivalent circuits (Figure \ref{fig:direct-graph-my})}\DIFdelend \DIFaddbegin \DIFadd{on its equivalent circuits }\DIFaddend .
...
\DIFaddbegin \DIFadd{Figure \ref{fig:direct-graph-my} shows the improved directed graph model used in this paper.
}\DIFaddend The following will provide a detailed explanation of the method for equating the components in RBS and constructing the directed graph model.
...
\DIFdelbegin \DIFdel{Based on the definition of MAC, determining the MAC of RBS }\DIFdelend \DIFaddbegin \DIFadd{For a given RBS, determination of its MAC }\DIFaddend involves maximizing the RBS output current while ensuring that the currents of all batteries do not exceed the batteries' maximum allowable current. 
In this subsection, the constraints and objective function to solve the RBS's MAC will be established through circuit analysis, based on \DIFdelbegin \DIFdel{the previously constructed }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend directed graph model \DIFaddbegin \DIFadd{provided in the previous subsection}\DIFaddend .
...
The $\bm{Y}$ in \DIFaddbegin \DIFadd{Equation }\DIFaddend \ref{eq:generalized_branches} is the admittance matrix of the circuit, defined as the inverse of the impedance matrix.
\DIFdelbegin \DIFdel{That is the elements of the diagonal }\DIFdelend \DIFaddbegin \DIFadd{The elements on the diagonal of }\DIFaddend matrix $\bm{Y}$ are equal to the reciprocal of the \DIFaddbegin \DIFadd{resistance, which is the }\DIFaddend second attribute of the corresponding edges in the directed graph, and the off-diagonal elements \DIFaddbegin \DIFadd{$\bm{Y}$ }\DIFaddend are 0.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.2\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{stru-L-origin}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{stru-L-origin.png}
        \DIFaddendFL \caption{}
...
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{stru-V-origin.png}
    ...
    \hspace{0.02\textwidth}
    \begin{subfigure}[b]{0.31\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{stru-my-origin}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{stru-my-origin.png}
        \DIFaddendFL \caption{}
...
Currently, two types of RBS structures have been proposed by Visairo et al. \cite{visairoReconfigurableBatteryPack2008} and Lawson et al. \cite{lawsonSoftwareConfigurableBattery2012}, both of which have been \DIFdelbegin \DIFdel{applied in practice}\DIFdelend \DIFaddbegin \DIFadd{practically used}\DIFaddend . 
The primary goal of Visairo's structure (Figure \ref{fig:study-stru-Visairo}) was to achieve dynamic adjustment of RBS output \DIFaddbegin \DIFadd{power}\DIFaddend ; however, the isolation of unhealthy batteries was not sufficiently addressed \DIFdelbegin \DIFdel{. 
...
Lawson et al. \DIFdelbegin \DIFdel{conducted research on battery isolation in RBS and specifically designed the }\DIFdelend \DIFaddbegin \DIFadd{designed the RBS }\DIFaddend structure shown in Figure \ref{fig:study-stru-Lawson} \DIFaddbegin \DIFadd{for the purpose of battery isolation}\DIFaddend . 
...
Based on the structures of Visairo and Lawson, this paper presents a new structure, as shown in Figure \ref{fig:study-stru-my}\DIFdelbegin \DIFdel{, which combines the advantages of both}\DIFdelend .
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.28\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{ef-topo}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{ef-topo.png}
        \DIFaddendFL \caption{}
        \label{fig:study-dirgraph-my}
    \end{subfigure}
    \hspace{0.05\textwidth}
    \begin{subfigure}[b]{0.28\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{ef-sp1}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{ef-sp1.png}
        \DIFaddendFL \caption{}
        \label{fig:sp1}
    \end{subfigure}
    \hspace{0.05\textwidth}
    \begin{subfigure}[b]{0.28\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{ef-sp2}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{ef-sp2.png}
        \DIFaddendFL \caption{}
        \label{fig:sp2}
    \end{subfigure}
    \\
    \begin{subfigure}[b]{0.28\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{ef-sp3}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{ef-sp3.png}
        \DIFaddendFL \caption{}
        \label{fig:sp3}
    \end{subfigure}
    \hspace{0.05\textwidth}
    \begin{subfigure}[b]{0.28\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{ef-sp4}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{ef-sp4.png}
        \DIFaddendFL \caption{}
        \label{fig:sp4}
    \end{subfigure}
    \hspace{0.05\textwidth}
    \begin{subfigure}[b]{0.28\textwidth}
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{ef-mac}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{ef-mac.png}
        \DIFaddendFL \caption{}
...
\begin{table}[htbp]
  \centering
    ...
    \DIFaddbeginFL \DIFaddFL{$\max$}\DIFaddendFL $\eta$     & 2 \\
    \bottomrule
    \end{tabular}
...
\DIFaddbegin \DIFadd{In order to verify and compare the results from the greedy algorithm, we also utilized a brute force algorithm that go through all possible switch states to calculate the MAC of the same three RBS. 
...
It is worth noting that the method used the greedy algorithm only calculated 11, 11, and 1 reconfigured structures for the RBS structure in Figures \ref{fig:study-stru-my}, \ref{fig:study-stru-Lawson}, \ref{fig:study-stru-Visairo}, respectively. 
...
\DIFdelendFL \DIFaddbeginFL \footnotesize{$^{\mathrm{b}}$ isolate one battery in each of the two substructures, as shown in Figure \ref{fig:my-isolated-2w}}
  ...
  \begin{figure}[htbp]
      \centering
      \begin{subfigure}[b]{0.31\textwidth}
          \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{my-isolated-1}
%DIFDELCMD <           %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{my-isolated-1.png}
          \DIFaddendFL \caption{}
          \label{fig:my-isolated-1}
      \end{subfigure}
      \hspace{0.02\textwidth}
      \begin{subfigure}[b]{0.31\textwidth}
          \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{my-isolated-2b}
%DIFDELCMD <           %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{my-isolated-2b.png}
          \DIFaddendFL \caption{}
          \label{fig:my-isolated-2b}
      \end{subfigure}
      \\
      \begin{subfigure}[b]{0.31\textwidth}
          \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{my-isolated-2w}
%DIFDELCMD <           %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{my-isolated-2w.png}
          \DIFaddendFL \caption{}
          \label{fig:my-isolated-2w}
      \end{subfigure}
      \hspace{0.02\textwidth}
      \begin{subfigure}[b]{0.31\textwidth}
          \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{my-isolated-3}
%DIFDELCMD <           %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{my-isolated-3.png}
          \DIFaddendFL \caption{}
...
\DIFdelbegin \DIFdel{In this subsection, we firstly discuss the correctness of the results presented }\DIFdelend \DIFaddbegin \DIFadd{As shown }\DIFaddend in Figure \ref{fig:all-results-my} and Table \ref{tab:study-results-my}.
...
\DIFaddbegin \DIFadd{The results of MAC obtained by brute force algorithm method is identical to the one by the greedy algorithm.
}\DIFaddend 


\DIFdelbegin \DIFdel{It is important to note that when solving for MAC}\DIFdelend \DIFaddbegin \DIFadd{From the literature research we have conducted, no formal report on the algorithm for MAC in RBS has been found yet.
...
, the number of batteries $N_b$, switches $N_s$, and nodes $N$ have the following quantitative relationships: $N_s \approx (3\sim 5)N_b$}\DIFaddend , \DIFaddbegin \DIFadd{$N \approx N_s$. 
...
Compared to the brute force algorithm, the efficiency of the method based on the greedy algorithm has been improved by 3000 to 48000 times, which is theoretically $N_s 2^{N_s - N_b} \log N_b$ times according to the above time complexity analysis.
...
\DIFadd{It is important to note that }\DIFaddend $\eta$ is used as the objective function instead of $I_o$ \DIFaddbegin \DIFadd{in solving MAC}\DIFaddend . 
...
\DIFdelbegin \DIFdel{If }\DIFdelend \DIFaddbegin \DIFadd{However, when }\DIFaddend $I_o$ \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend used as the objective function, even for the same RBS structure, the MAC \DIFdelbegin \DIFdel{result }\DIFdelend \DIFaddbegin \DIFadd{solution }\DIFaddend and corresponding switches state could change due to different external electrical appliances.
It would increase the difficulty and uncertainty in RBS structure design. 
In \DIFdelbegin \DIFdel{contrast, by using }\DIFdelend \DIFaddbegin \DIFadd{order to eliminate the influences of such a problem, }\DIFaddend $\eta$\DIFdelbegin \DIFdel{as the objective function}\DIFdelend , which is \DIFdelbegin \DIFdel{defined as }\DIFdelend the ratio of $I_o$ and $\max\bm{I}_b$, \DIFdelbegin \DIFdel{the influence of these factors on the results can be eliminated}\DIFdelend \DIFaddbegin \DIFadd{is adopted as the objective function in our research}\DIFaddend .
...
\DIFdelbegin \DIFdel{Therefore, compared to $I_o$, $\eta$ is more suitable for structure design.
}\DIFdelend 


The method proposed in this paper is significant for the design of \DIFdelbegin \DIFdel{next-generation }\DIFdelend RBSs in the following aspects.
...
Moreover, \DIFdelbegin \DIFdel{isolating }\DIFdelend \DIFaddbegin \DIFadd{isolation of }\DIFaddend the batteries disrupts the original regularity and symmetry of the topology, which complicates the otherwise simple structure, and the maximum output current of the system becomes more challenging to obtain.
\DIFdelbegin \DIFdel{Owing to the advantages of pervasiveness and automation}\DIFdelend \DIFaddbegin \DIFadd{On the contrary}\DIFaddend , the proposed method can be employed to calculate the MAC of arbitrary RBS structures, \DIFdelbegin \DIFdel{which helps to address the aforementioned issues and paves the way for more }\DIFdelend \DIFaddbegin \DIFadd{especially for these above }\DIFaddend complex and flexible RBS \DIFdelbegin \DIFdel{structure design}\DIFdelend \DIFaddbegin \DIFadd{structures}\DIFaddend .


To illustrate this point, the MACs of \DIFdelbegin \DIFdel{the }\DIFdelend three RBS structures mentioned above are calculated after \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{one and more }\DIFaddend batteries are isolated, as shown in Table \ref{tab:isolated_mac}. 
Specifically, for the structure presented in Figure \ref{fig:study-stru-my}, the corresponding circuit states of MACs \DIFdelbegin \DIFdel{when isolating different numbers of }\DIFdelend \DIFaddbegin \DIFadd{of isolating one to three }\DIFaddend batteries are depicted in Figures \ref{fig:my-isolated-1}-\ref{fig:my-isolated-3}. 
...
From the results \DIFaddbegin \DIFadd{shown in Figure \ref{fig:my-isolated-1}-\ref{fig:my-isolated-3}}\DIFaddend , it can be observed that the proposed method provides reasonable outcomes for isolating batteries with any number and position.


Furthermore, the \DIFdelbegin \DIFdel{performance of }\DIFdelend output current for the three RBS \DIFdelbegin \DIFdel{when isolating }\DIFdelend \DIFaddbegin \DIFadd{with isolated }\DIFaddend batteries is also shown in Table \ref{tab:isolated_mac}. 
For the structure proposed by Lawson et al., the MAC \DIFdelbegin \DIFdel{remains the same as that without isolated battery cells, i.e., $\eta=1$, when the }\DIFdelend \DIFaddbegin \DIFadd{is independent on the }\DIFaddend number of isolated \DIFdelbegin \DIFdel{battery cells increases, until all the cells in the RBS are isolated.
For }\DIFdelend \DIFaddbegin \DIFadd{batteries.
However, for }\DIFaddend Visairo's structure, the MAC decreases \DIFdelbegin \DIFdel{as the }\DIFdelend \DIFaddbegin \DIFadd{with the increasing }\DIFaddend number of isolated \DIFdelbegin \DIFdel{battery cells increases, until $\eta=0$. 
In contrast}\DIFdelend \DIFaddbegin \DIFadd{batteries. 
Nevertherless}\DIFaddend , the MAC of the structure proposed in this work is positioned between \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{these above }\DIFaddend two structures. 
This indicates that the structure proposed in this paper, compared to Lawson's structure, has a larger MAC under the same number of batteries, \DIFdelbegin \DIFdel{which means }\DIFdelend \DIFaddbegin \DIFadd{and exhibits }\DIFaddend a wider output current regulation range. 
\DIFdelbegin \DIFdel{On the other hand, by simply changing the states of $S_2$, $S_4$, $S_{11}$, and $S_{12}$ in the conversion structure, this structure can address the majority of battery isolation scenarios, whereas Visairo's structure requires specific battery targeting and switch control. 
In summary, the structure proposed in this paper has the advantages of both Lawson's and Visairo's structures.
}\DIFdelend 

\section{Conclusion}

This paper proposes a pervasive and automatical method for computing the MAC of \DIFdelbegin \DIFdel{the given RBS }\DIFdelend \DIFaddbegin \DIFadd{a given RBS efficiently}\DIFaddend .
...
Future research could focus on developing new indictors to evaluate the performance of the RBS with the currents and voltages obtained by the method, as well as modifying the equivalent model of the battery to allow for more accurate simulations of the RBS , including transient analysis}\DIFdelend \DIFaddbegin \DIFadd{which considers the voltage, internal resistance, and maximum allowable current of the batteries, as well as the external load.
...
This method helps to fully tap the current output potential of the RBS, guide the RBS structure design and optimization in the design stage, and assist in evaluating the current overload risk of the system in practical applications}\DIFaddend .
