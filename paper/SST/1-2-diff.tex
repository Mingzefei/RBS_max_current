...
%DIF DEL 1-main.tex   Tue Sep 19 22:57:17 2023
%DIF ADD 2-main.tex   Wed Sep 20 11:42:14 2023
...
%DIF < \addbibresource{ref.bib}
%DIF -------
% \usepackage[style=nejm,  %DIF > 
% citestyle=numeric-comp,  %DIF > 
% sorting=none]{biblatex} %DIF > 
% \addbibresource{ref.bib} %DIF > 
%DIF -------
...
During \DIFdelbegin \DIFdel{the operation of the RBS , the Maximum Allowable Current }\DIFdelend \DIFaddbegin \DIFadd{RBS operation, a critical system parameter known as the maximum allowable current }\DIFaddend (MAC) \DIFdelbegin \DIFdel{of system that ensures each battery’s current remains }\DIFdelend \DIFaddbegin \DIFadd{become pivotal. This parameter is instrumental in maintaining the current of each individual battery }\DIFaddend within a safe range \DIFdelbegin \DIFdel{, is a critical indicator to guide the system’s reconfiguring control, }\DIFdelend \DIFaddbegin \DIFadd{and serves as a guiding indicator for the system's reconfiguration, thereby }\DIFaddend ensuring its safety and reliability.
\DIFdelbegin \DIFdel{In this paper , we firstly propose a calculation method for }\DIFdelend %DIF > <*reviewer1-comment1>
\DIFaddbegin \DIFadd{This paper proposes a method to calculate }\DIFaddend the MAC of arbitrary \DIFdelbegin \DIFdel{RBS }\DIFdelend \DIFaddbegin \DIFadd{RBSs }\DIFaddend using a greedy algorithm in conjunction with a directed graph model of the RBS.
...
The }\DIFaddend proposed method is validated \DIFdelbegin \DIFdel{by a novel and complex RBS }\DIFdelend \DIFaddbegin \DIFadd{on two published four-battery-RBSs and one with a more complex }\DIFaddend structure.
The results \DIFdelbegin \DIFdel{show that this method is capable to calculated }\DIFdelend \DIFaddbegin \DIFadd{are the same as those of the brute-force algorithm, but the proposed method significantly improves the computational efficiency ($N_s 2^{N_s - N_b} \log_{10} N_b$ times faster than the brute force algorithm for an RBS with $N_b$ batteries and $N_s$ switches, theoretically).
...
}\DIFdelend \DIFaddbegin \DIFadd{arbitrary structures, even in scenarios with random isolated batteries.
...
Battery \DIFdelbegin \DIFdel{Energy Storage Systems }\DIFdelend \DIFaddbegin \DIFadd{energy storage systems }\DIFaddend (BESSs) are extensively \DIFdelbegin \DIFdel{employed }\DIFdelend \DIFaddbegin \DIFadd{used }\DIFaddend in various applications \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{yangBatteryEnergyStorage2018}}\hskip0pt%DIFAUXCMD
}\DIFaddend , such as wind power plants \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{desiqueiraControlStrategySmooth2021} }\hskip0pt%DIFAUXCMD
}\DIFaddend and space power systems \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{schwanbeckInternationalSpaceStation2019,zhangDevelopmentProspectChinese2021}}\hskip0pt%DIFAUXCMD
}\DIFaddend , to store and release high-quality electrical energy \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{desiqueiraControlStrategySmooth2021,yangBatteryEnergyStorage2018,choCommercialResearchBattery2015,zhangDevelopmentProspectChinese2021,schwanbeckInternationalSpaceStation2019}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{choCommercialResearchBattery2015}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
These problems have become significant technical barriers in \DIFdelbegin \DIFdel{the development of }\DIFdelend \DIFaddbegin \DIFadd{many engineering projects requiring high reliability, such as developing }\DIFaddend new-generation space vehicles \DIFdelbegin \DIFdel{and urgently need to be addressed }\DIFdelend \cite{pomboHybridPowerSystem2021}. 
...
\DIFdelbegin \DIFdel{The complex connection }\DIFdelend %DIF > <*reviewer1-comment2>
\DIFaddbegin \DIFadd{Recently, various types of RBSs with different flexibility and reconfigurability have been designed to meet application requirements. 
...
In this study, a directed graph model and greedy algorithm are employed to determine the MAC of RBS and the corresponding control strategy, effectively calculating the MAC for }\DIFdelend \DIFaddbegin \DIFadd{allowed current  under the constraints of the battery cell, is another critical indicator of RBSs that needs to be evaluated during the design or control  of the system. 
...
The main contributions of this paper can be summarized as follows:
  ...
  \item \DIFadd{An efficient method is proposed to determine the MAC of }\DIFaddend RBSs with arbitrary structures, including scenarios with isolated batteries.
  \DIFaddbegin \item \DIFadd{A greedy algorithm is applied to solve the MAC problem, the computational complexity of which is greatly reduced compared with the brute-force algorithm.
  }\item \DIFadd{An improved directed graph model is introduced to provide a specific method for calculating the MAC of a given structure.
}\end{itemize}
%DIF > </reviewer1-comment3>
\DIFaddend 
...
Section III \DIFdelbegin \DIFdel{demonstrated }\DIFdelend \DIFaddbegin \DIFadd{discusses }\DIFaddend a case study \DIFdelbegin \DIFdel{of using }\DIFdelend \DIFaddbegin \DIFadd{that uses }\DIFaddend the proposed method to determine the \DIFdelbegin \DIFdel{MAC of a novel and }\DIFdelend \DIFaddbegin \DIFadd{MACs of two published four-battery RBSs and one with a more }\DIFaddend complex structure. 
The calculation results\DIFaddbegin \DIFadd{, the algorithm's computational complexity, }\DIFaddend and scenarios such as \DIFdelbegin \DIFdel{batteries isolation also are }\DIFdelend \DIFaddbegin \DIFadd{battery random isolation are also }\DIFaddend discussed. 
Finally, the concluding remarks are \DIFdelbegin \DIFdel{drawn }\DIFdelend \DIFaddbegin \DIFadd{presented }\DIFaddend in Section IV.
...
\DIFdelbegin \DIFdel{Then, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend shortest paths (SPs, where additional batteries and switches on the path are penalized as distance) for the batteries are \DIFdelbegin \DIFdel{obtained }\DIFdelend \DIFaddbegin \DIFadd{then obtained  by }\DIFaddend using the Dijkstra algorithm to \DIFdelbegin \DIFdel{guide }\DIFdelend \DIFaddbegin \DIFadd{connect }\DIFaddend the batteries in the RBS \DIFdelbegin \DIFdel{connect }\DIFdelend in parallel.
Finally, a greedy algorithm is \DIFdelbegin \DIFdel{employed }\DIFdelend \DIFaddbegin \DIFadd{used }\DIFaddend to organize the switches, allowing the batteries to connect via their SPs while satisfying the constraints, resulting in the MAC of the RBS.
...
\DIFdelbegin \DIFdel{As shown in Figure }\DIFdelend \DIFaddbegin \DIFadd{For instance, as shown in Fig. }\DIFaddend \ref{fig:battery_simple}, the battery in the RBS \DIFdelbegin \DIFdel{can be }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend represented as a black-box circuit consisting of two resistors \DIFdelbegin \DIFdel{(i.e., }\DIFdelend $r_1$ and $r_2$ \DIFdelbegin \DIFdel{) }\DIFdelend and a capacitor \DIFdelbegin \DIFdel{(i.e., }\DIFdelend $C$\DIFdelbegin \DIFdel{)}\DIFdelend , known as the Thevenin model \cite{hongwenheStateofChargeEstimationLithiumIon2011,mousavig.VariousBatteryModels2014}.
...
Therefore,  \DIFdelbegin \DIFdel{the battery $i$ }\DIFdelend \DIFaddbegin \DIFadd{battery $B_i$ }\DIFaddend in the RBS can be simplified as \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend series connection between a constant voltage source $u_{i}$ and a resistor $r_{i}$.
Furthermore, the state of switch \DIFdelbegin \DIFdel{$j$ }\DIFdelend \DIFaddbegin \DIFadd{$S_j$ }\DIFaddend in the RBS is represented by a binary variable $x_j$, where 0 is \DIFdelbegin \DIFdel{for }\DIFdelend ON and 1 is \DIFdelbegin \DIFdel{for OFF, respectively}\DIFdelend \DIFaddbegin \DIFadd{OFF}\DIFaddend .
...
\begin{enumerate}
    ...
    \item Edges:
        ...
        For \DIFdelbegin \DIFdel{a }\DIFdelend battery $B_i$, its directed edge $e_i$ is drawn from the cathode to the anode \DIFdelbegin \DIFdel{, as the battery }\DIFdelend \DIFaddbegin \DIFadd{because the battery in operation }\DIFaddend only allows current to flow in one direction\DIFdelbegin \DIFdel{when in operation.
        For a }\DIFdelend \DIFaddbegin \DIFadd{.
        For }\DIFaddend switch $S_j$, since it is allowed to work under \DIFdelbegin \DIFdel{bi-directional }\DIFdelend \DIFaddbegin \DIFadd{bidirectional }\DIFaddend currents, it is represented by a pair of directed edges with two-way directions. 
...
\DIFdelbegin \DIFdel{Based on the definition of MAC, determining the MAC of RBS }\DIFdelend \DIFaddbegin \DIFadd{For a given RBS, determining its MAC }\DIFaddend involves maximizing the RBS output current while ensuring that \DIFdelbegin \DIFdel{the currents of all batteries }\DIFdelend \DIFaddbegin \DIFadd{all battery currents }\DIFaddend do not exceed the batteries' \DIFdelbegin \DIFdel{maximum allowable current. 
...
\DIFdelbegin \DIFdel{:
    ...
    \end{bmatrix},
\end{equation}
where $\bm{A}_b$, $\bm{A}_s$\DIFaddbegin \DIFadd{, }\DIFaddend and $\bm{A}_o$ are the \DIFdelbegin \DIFdel{sub-matrices }\DIFdelend \DIFaddbegin \DIFadd{submatrices }\DIFaddend corresponding to the batteries, switches\DIFaddbegin \DIFadd{, }\DIFaddend and external electrical load, respectively.
...
According to \DIFdelbegin \DIFdel{Kirchhoffs law}\DIFdelend \DIFaddbegin \DIFadd{Kirchhoff's laws}\DIFaddend , we have
...
Because all batteries have been equivalent to voltage sources rather than current sources in the previous subsection, all elements of the array $\bm{I}_s$ are \DIFdelbegin \DIFdel{0, 
...
\DIFdelbegin \DIFdel{Where the elements }\DIFdelend \DIFaddbegin \DIFadd{where element }\DIFaddend $x_i$ of \DIFdelbegin \DIFdel{the }\DIFdelend matrix $\bm{X}_b$ \DIFdelbegin \DIFdel{represent whether the battery $i$ }\DIFdelend \DIFaddbegin \DIFadd{indicates whether battery $B_i$ }\DIFaddend has been removed from the circuit, with $x_i=1$ indicating removal and $x_i=0$ indicating that \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{battery $B_i$ }\DIFaddend is still available to supply power. 
...
The elements $x_j$ of  \DIFdelbegin \DIFdel{the }\DIFdelend matrix $\bm{X}_s$ \DIFdelbegin \DIFdel{represent whether the switch $j$ }\DIFdelend \DIFaddbegin \DIFadd{determine whether switch $S_j$ }\DIFaddend is closed, with $x_j=1$ indicating \DIFdelbegin \DIFdel{closure }\DIFdelend \DIFaddbegin \DIFadd{a closed switch }\DIFaddend and $x_j=0$ indicating \DIFdelbegin \DIFdel{disconnection}\DIFdelend \DIFaddbegin \DIFadd{an open switch}\DIFaddend , which is consistent with the previous subsection.
...
\DIFdelbegin \DIFdel{In order to obtain specific constraint conditions and objective functions}\DIFdelend \DIFaddbegin \DIFadd{To further simplify the problem}\DIFaddend , it is \DIFdelbegin \DIFdel{further }\DIFdelend assumed that all batteries have the same electromotive force and internal resistance, \DIFdelbegin \DIFdel{denoted as }\DIFdelend \DIFaddbegin \DIFadd{which are denoted }\DIFaddend $u_b$ and $r_b$, respectively.
...
The detailed implementation \DIFdelbegin \DIFdel{process }\DIFdelend \DIFaddbegin \DIFadd{of this algorithm }\DIFaddend is presented in the following two subsections.
...
Moreover, the shortest path $SP_i$ is defined as the path with the minimum $\omega$ for battery \DIFdelbegin \DIFdel{$i$, as shown in the following equation}\DIFdelend \DIFaddbegin \DIFadd{$B_i$}\DIFaddend :
...
Denoting }\DIFaddend the cathode and anode of battery \DIFdelbegin \DIFdel{$i$ are denoted }\DIFdelend \DIFaddbegin \DIFadd{$B_i$ }\DIFaddend as $v_i^-$ and $v_i^+$ respectively, \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{then }\DIFaddend path $p$ of battery \DIFdelbegin \DIFdel{$i$  }\DIFdelend \DIFaddbegin \DIFadd{$B_i$  }\DIFaddend can be divided into three segments: $v_1 \rightarrow v_i^-$, $v_i^+ \rightarrow v_N$, and $v_i^- \rightarrow v_i^+$. \DIFdelbegin \DIFdel{The }\DIFdelend $v_i^- \rightarrow v_i^+$ is the directed edge corresponding to battery \DIFdelbegin \DIFdel{$i$}\DIFdelend \DIFaddbegin \DIFadd{$B_i$}\DIFaddend . 
...
Finally, \DIFdelbegin \DIFdel{the }\DIFdelend $SP_i$ for battery \DIFdelbegin \DIFdel{$i$ }\DIFdelend \DIFaddbegin \DIFadd{$B_i$ }\DIFaddend is formed by the complete path\DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{, which consists of }\DIFaddend $SP(v_1 \rightarrow v_i^-)$, $v_i^- \rightarrow v_i^+$, and $SP(v_i^+ \rightarrow v_N)$.
...
From the perspective of series \DIFdelbegin \DIFdel{/}\DIFdelend \DIFaddbegin \DIFadd{vs }\DIFaddend parallel connections, integrating more batteries into the circuit through their shortest paths (\DIFdelbegin \DIFdel{$SP$s}\DIFdelend \DIFaddbegin \DIFadd{SPs}\DIFaddend ) results in \DIFdelbegin \DIFdel{a larger number of }\DIFdelend \DIFaddbegin \DIFadd{more }\DIFaddend batteries connected in parallel, thereby increasing the total output current of the RBS.
However, conflicts may arise between the \DIFdelbegin \DIFdel{$SP$s }\DIFdelend \DIFaddbegin \DIFadd{SPs }\DIFaddend of different batteries. 
For instance, the \DIFdelbegin \DIFdel{$SP$s }\DIFdelend \DIFaddbegin \DIFadd{SPs }\DIFaddend of two batteries might form a \DIFdelbegin \DIFdel{short-circuited }\DIFdelend \DIFaddbegin \DIFadd{short-circuit }\DIFaddend RBS structure, which is not allowed. 
To address this issue, a greedy algorithm \DIFdelbegin \DIFdel{is employed to incorporate as many $SP$s }\DIFdelend \DIFaddbegin \DIFadd{incorporates as many SPs }\DIFaddend as possible while satisfying the reconstruction requirements.
...
}\DIFaddend First, the \DIFdelbegin \DIFdel{shortest paths ($SP$s) are obtained using Equations \ref{eq:weight}and \ref{eq:def_sp} }\DIFdelend \DIFaddbegin \DIFadd{SPs are obtained by using Eqs. (\ref{eq:weight}) and (\ref{eq:def_sp}) }\DIFaddend in conjunction with \DIFdelbegin \DIFdel{Dijkstra Search}\DIFdelend \DIFaddbegin \DIFadd{the Dijkstra search}\DIFaddend . 
...
\DIFdelbegin \DIFdel{And their variations in output current under battery isolation conditions will be studied.
This RBS structure will be used to validate the effectiveness of the proposed method for calculating the MAC, and be compared with the Lawson's and Visairo's structure to illustrate its advantage on battery isolation}\DIFdelend \DIFaddbegin \DIFadd{These four-battery RBS structures are investigated in the case study, including the scenarios with random isolated batteries}\DIFaddend .
...
\DIFdelbegin \DIFdel{Utilizing Equation \ref{eq:weight}}\DIFdelend \DIFaddbegin \DIFadd{Using Eq. (\ref{eq:weight}) }\DIFaddend and the Dijkstra algorithm, the \DIFdelbegin \DIFdel{$SP$s }\DIFdelend \DIFaddbegin \DIFadd{SPs }\DIFaddend of the four batteries in the RBS structure of \DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:study-stru-my} are highlighted \DIFdelbegin \DIFdel{by red in Figures \ref{fig:sp1} -}\DIFdelend \DIFaddbegin \DIFadd{in red in Figs. \ref{fig:sp1} and }\DIFaddend \ref{fig:sp4}.
Finally, the \DIFdelbegin \DIFdel{MAC calculation results }\DIFdelend \DIFaddbegin \DIFadd{calculated MACs }\DIFaddend of the structure in \DIFdelbegin \DIFdel{Figure \ref{fig:study-stru-my} are shown as Table \ref{tab:study-results-my} and Figure \ref{fig:study-results-my}, }\DIFdelend \DIFaddbegin \DIFadd{Fig. \ref{fig:study-stru-my} are listed in Tab. \ref{tab:study-results-my} and shown in Fig. \ref{fig:study-results-my}, as }\DIFaddend obtained by the greedy algorithm \ref{alg:greedy}.
\DIFdelbegin \DIFdel{Table }\DIFdelend \DIFaddbegin \DIFadd{Tab. }\DIFaddend \ref{tab:study-results-my} contains the \DIFdelbegin \DIFdel{switches states }\DIFdelend \DIFaddbegin \DIFadd{states of the switches}\DIFaddend , the output current $I_o$, \DIFaddbegin \DIFadd{the }\DIFaddend battery current $\bm{I}_b$\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, and the }\DIFaddend ratio $\eta$ of the RBS structure with all batteries in good health when the RBS output reaches the MAC.
\DIFdelbegin \DIFdel{Figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:study-results-my} presents the corresponding circuit, with the red highlight indicating that \DIFaddbegin \DIFadd{the }\DIFaddend current is flowing through the respective branches.
...
\begin{figure}[htbp]
    ...
    \caption{
        ...
        and the \DIFdelbeginFL \DIFdelFL{$SP$s }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{SPs }\DIFaddendFL (highlighted in red) of battery (b) $B_1$, (c) $B_2$, (d) $B_3$, \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL (e) $B_4$.
...
Similarly, the \DIFdelbegin \DIFdel{MAC calculation }\DIFdelend results of the \DIFdelbegin \DIFdel{structures in Figures }\DIFdelend \DIFaddbegin \DIFadd{MAC calculation for the structures in Figs. }\DIFaddend \ref{fig:study-stru-Lawson} and \ref{fig:study-stru-Visairo} are \DIFdelbegin \DIFdel{shown as Table \ref{tab:study-results-Lawson} and  Table }\DIFdelend \DIFaddbegin \DIFadd{listed in Tabs. \ref{tab:study-results-Lawson} and  }\DIFaddend \ref{tab:study-results-Visairo}, respectively.
...
\DIFaddend \begin{table}[htbp]
  \centering
    \caption{MAC Calculating result of the \DIFdelbeginFL \DIFdelFL{4-battery }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{four-battery }\DIFaddendFL RBS structure in \DIFdelbeginFL \DIFdelFL{Figure }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Fig. }\DIFaddendFL \ref{fig:study-stru-Lawson}.}
    \begin{tabular}{cc}
    \toprule
...
\begin{table}[htbp]
  \centering
    \caption{MAC Calculating result of the \DIFdelbeginFL \DIFdelFL{4-battery }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{four-battery }\DIFaddendFL RBS structure in \DIFdelbeginFL \DIFdelFL{Figure }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Fig. }\DIFaddendFL \ref{fig:study-stru-Visairo}.}
    \begin{tabular}{cc}
    \toprule
...
The MAC calculation results for the three structures under study, with varying numbers of isolated batteries, are presented in \DIFdelbegin \DIFdel{Table \ref{tab:isolated_mac}. 
Figures \ref{fig:my-isolated-1}-}\DIFdelend \DIFaddbegin \DIFadd{Tab. \ref{tab:isolated_mac}. 
Figs. \ref{fig:my-isolated-1}--}\DIFaddend \ref{fig:my-isolated-3} illustrate the corresponding \DIFdelbegin \DIFdel{switch control }\DIFdelend \DIFaddbegin \DIFadd{switch-control }\DIFaddend schemes for the new structure proposed in this paper under different \DIFdelbegin \DIFdel{isolated battery conditions .
...
\DIFdelbegin \DIFdel{In this subsection, we firstly discuss the correctness of the results presented in Figure \ref{fig:all-results-my} and Table }\DIFdelend \DIFaddbegin \DIFadd{Consider first the results shown in Fig. \ref{fig:study-results-my} and listed in Tab. }\DIFaddend \ref{tab:study-results-my}.
...
Therefore, the \DIFdelbegin \DIFdel{switches state given in Table \ref{tab:study-results-my} can make }\DIFdelend \DIFaddbegin \DIFadd{state of the switches given in Tab. \ref{tab:study-results-my} maximizes }\DIFaddend the RBS output current\DIFdelbegin \DIFdel{reach the maximum}\DIFdelend \DIFaddbegin \DIFadd{.
The brute-force method, which go through all possible switch states, also gives the same result}\DIFaddend .


\DIFdelbegin \DIFdel{It is important to note that when solving for MAC}\DIFdelend %DIF > <*reviewer1-comment4>
\DIFaddbegin \DIFadd{The literature contains no report on an algorithm for calculating the MAC of an RBS.
...
\item[\DIFadd{(1)}] \DIFadd{The SPs guide the RBS to reconfigure reasonable structures rather than blindly going through all possible structures. This reduces the complexity from $2^{N_s}$ to $2^{N_b}$, which is the main reason for the improvement in efficiency.
}\item[\DIFadd{(2)}] \DIFadd{The bisection method further accelerates this process, reducing the complexity from $2^{N_b}$ to $2^{N_b} N_B^{-1} \log_{10} N_b$.
}\end{enumerate}
%DIF > </reviewer1-comment4>
%DIF > <*reviewer1-comment5>
\DIFadd{However, the greedy algorithm proposed in this paper still contains exponential terms in the time complexity, which means it may not be able to handle extremely large RBS structures having large $N_b$.
%DIF > </reviewer1-comment5>
}
...
As shown in \DIFdelbegin \DIFdel{Table }\DIFdelend \DIFaddbegin \DIFadd{Tab. }\DIFaddend \ref{tab:study-results-my}, $I_o$ and $\bm{I}_b$ are functions of $R_o$, $u_b$, and $r_b$. 
...
To eliminate this problem, }\DIFaddend the \DIFdelbegin \DIFdel{maximum output currentcapability of the RBS structure}\DIFdelend \DIFaddbegin \DIFadd{ratio $\eta=I_o/\max\bm{I}_b$ is adopted as the objective function in our research.
Recall that $\eta$ reflects only the structure's ability to output current, rather than the actual current outputing by the battery system}\DIFaddend .
...
To illustrate this point, the MACs of \DIFdelbegin \DIFdel{the }\DIFdelend three RBS structures mentioned above are calculated after \DIFdelbegin \DIFdel{the batteries are isolated}\DIFdelend \DIFaddbegin \DIFadd{isolating one or more of the batteries}\DIFaddend , as shown in \DIFdelbegin \DIFdel{Table }\DIFdelend \DIFaddbegin \DIFadd{Tab. }\DIFaddend \ref{tab:isolated_mac}. 
...
\DIFdelbegin \DIFdel{From the results , it can be observed }\DIFdelend \DIFaddbegin \DIFadd{The results in Figs. \ref{fig:my-isolated-1}--\ref{fig:my-isolated-3} show }\DIFaddend that the proposed method provides reasonable outcomes for isolating \DIFdelbegin \DIFdel{batteries with any number and position.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{any number of batteries in any position.
}\DIFaddend Furthermore, the \DIFdelbegin \DIFdel{performance of }\DIFdelend output current for the three \DIFdelbegin \DIFdel{RBS when isolating }\DIFdelend \DIFaddbegin \DIFadd{RBSs with isolated }\DIFaddend batteries is also shown in \DIFdelbegin \DIFdel{Table }\DIFdelend \DIFaddbegin \DIFadd{Tab. }\DIFaddend \ref{tab:isolated_mac}. 
...
This paper proposes a pervasive and \DIFdelbegin \DIFdel{automatical method for computing }\DIFdelend \DIFaddbegin \DIFadd{automated method to efficiently compute }\DIFaddend the MAC of \DIFdelbegin \DIFdel{the given }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend RBS.
The method is implemented by a greedy algorithm combined with \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an improved }\DIFaddend directed graph model\DIFdelbegin \DIFdel{, whose effectiveness is tested on a novel and complex RBS structure.
...
Future research could focus on developing new indictors to evaluate the performance }\DIFdelend \DIFaddbegin \DIFadd{.
...
\begin{algorithm}
    ...
    get $\bm{A}$ by \DIFdelbegin \DIFdel{Equation }\DIFdelend \DIFaddbegin \DIFadd{Eq. }\DIFaddend \ref{eq:A}\;
    \While{not yet determine $\max \eta$ }
    {
        \DIFdelbegin \DIFdel{$N_{set} \leftarrow \text{number of setected $SP$s calculated by dichotomy}$}\DIFdelend \DIFaddbegin \DIFadd{$N_{\text{set}} \leftarrow$ number of setected SPs calculated by dichotomy}\DIFaddend \;
...
%DIF >  \bibliography{ref}
\DIFaddbegin 

\begin{thebibliography}{10}

\bibitem{yangBatteryEnergyStorage2018}
\DIFadd{Yuqing Yang, Stephen Bremner, Chris Menictas, and Merlinde Kay.
}\newblock \DIFadd{Battery energy storage system size determination in renewable energy systems: }{{\DIFadd{A}}} \DIFadd{review.
}\newblock {\em \DIFadd{Renewable and Sustainable Energy Reviews}}\DIFadd{, 91:109--125, August 2018.
}

\bibitem{desiqueiraControlStrategySmooth2021}
\DIFadd{Luanna Maria~Silva }{\DIFadd{de Siqueira}} \DIFadd{and Wei Peng.
}\newblock \DIFadd{Control strategy to smooth wind power output using battery energy storage system: }{{\DIFadd{A}}} \DIFadd{review.
}\newblock {\em \DIFadd{Journal of Energy Storage}}\DIFadd{, 35:102252, March 2021.
}

\bibitem{schwanbeckInternationalSpaceStation2019}
\DIFadd{Eugene Schwanbeck and Penni Dalton.
}\newblock \DIFadd{International }{{\DIFadd{Space Station Lithium-ion Batteries}}} \DIFadd{for }{{\DIFadd{Primary Electric Power System}}}\DIFadd{.
}\newblock \DIFadd{In }{\em \DIFadd{2019 }{{\DIFadd{European Space Power Conference}}} \DIFadd{(}{{\DIFadd{ESPC}}}\DIFadd{)}}\DIFadd{, pages 1--1. }{\DIFadd{IEEE}}\DIFadd{, September 2019.
}

\bibitem{zhangDevelopmentProspectChinese2021}
\DIFadd{Lihua Zhang.
}\newblock \DIFadd{Development and }{{\DIFadd{Prospect}}} \DIFadd{of }{{\DIFadd{Chinese Lunar Relay Communication Satellite}}}\DIFadd{.
}\newblock {\em \DIFadd{Space: Science \& Technology}}\DIFadd{, 2021, January 2021.
}

\bibitem{choCommercialResearchBattery2015}
\DIFadd{Jaephil Cho, Sookyung Jeong, and Youngsik Kim.
}\newblock \DIFadd{Commercial and research battery technologies for electrical energy storage applications.
}\newblock {\em \DIFadd{Progress in Energy and Combustion Science}}\DIFadd{, 48:84--101, June 2015.
}

\bibitem{yangUnbalancedDischargingAging2016}
\DIFadd{Naixing Yang, Xiongwen Zhang, BinBin Shang, and Guojun Li.
}\newblock \DIFadd{Unbalanced discharging and aging due to temperature differences among the cells in a lithium-ion battery pack with parallel combination.
}\newblock {\em \DIFadd{Journal of Power Sources}}\DIFadd{, 306:733--741, February 2016.
}

\bibitem{fengPropagationMechanismsDiagnosis2019}
\DIFadd{Fei Feng, Xiaosong Hu, Lin Hu, Fengling Hu, Yang Li, and Lei Zhang.
}\newblock \DIFadd{Propagation mechanisms and diagnosis of parameter inconsistency within }{{\DIFadd{Li-Ion}}} \DIFadd{battery packs.
}\newblock {\em \DIFadd{Renewable and Sustainable Energy Reviews}}\DIFadd{, 112:102--113, September 2019.
}

\bibitem{jeevarajanBatterySafetyQualifications2012}
\DIFadd{J.~A. Jeevarajan and C.~Winchester.
}\newblock \DIFadd{Battery }{{\DIFadd{Safety Qualifications}}} \DIFadd{for }{{\DIFadd{Human Ratings}}}\DIFadd{.
}\newblock {\em \DIFadd{Interface magazine}}\DIFadd{, 21(2):51--55, January 2012.
}

\bibitem{pomboHybridPowerSystem2021}
\DIFadd{Daniel~V}{\DIFadd{\'a}}\DIFadd{zquez Pombo.
}\newblock \DIFadd{A }{{\DIFadd{Hybrid Power System}}} \DIFadd{for a }{{\DIFadd{Permanent Colony}}} \DIFadd{on }{{\DIFadd{Mars}}}\DIFadd{.
}\newblock {\em \DIFadd{Space: Science \& Technology}}\DIFadd{, 2021, January 2021.
}

\bibitem{hanNextGenerationBatteryManagement2020a}
\DIFadd{Weiji Han, Torsten Wik, Anton Kersten, Guangzhong Dong, and Changfu Zou.
}\newblock \DIFadd{Next-}{{\DIFadd{Generation Battery Management Systems}}}\DIFadd{: }{{\DIFadd{Dynamic Reconfiguration}}}\DIFadd{.
}\newblock {\em \DIFadd{IEEE Industrial Electronics Magazine}}\DIFadd{, 14(4):20--31, December 2020.
}

\bibitem{visairoReconfigurableBatteryPack2008}
\DIFadd{H.~Visairo and P.~Kumar.
}\newblock \DIFadd{A reconfigurable battery pack for improving power conversion efficiency in portable devices.
}\newblock \DIFadd{In }{\em \DIFadd{2008 7th }{{\DIFadd{International Caribbean Conference}}} \DIFadd{on }{{\DIFadd{Devices}}}\DIFadd{, }{{\DIFadd{Circuits}}} \DIFadd{and }{{\DIFadd{Systems}}}}\DIFadd{, pages 1--6. }{\DIFadd{IEEE}}\DIFadd{, April 2008.
}

\bibitem{ci2007novel}
\DIFadd{Song Ci, Jiucai Zhang, Hamid Sharif, and Mahmoud Alahmad.
}\newblock \DIFadd{A novel design of adaptive reconfigurable multicell battery for power-aware embedded networked sensing systems.
}\newblock \DIFadd{In }{\em \DIFadd{IEEE GLOBECOM 2007-IEEE Global Telecommunications Conference}}\DIFadd{, pages 1043--1047. IEEE, 2007.
}

\bibitem{9209774}
\DIFadd{Jan Engelhardt, Tatiana Gabderakhmanova, Gunnar Rohde, and Mattia Marinelli.
}\newblock \DIFadd{Reconfigurable stationary battery with adaptive cell switching for electric vehicle fast-charging.
}\newblock \DIFadd{In }{\em \DIFadd{2020 55th International Universities Power Engineering Conference (UPEC)}}\DIFadd{, pages 1--6, 2020.
}

\bibitem{engelhardt2021double}
\DIFadd{Jan Engelhardt, Jan~Martin Zepter, Tatiana Gabderakhmanova, Gunnar Rohde, and Mattia Marinelli.
}\newblock \DIFadd{Double-string battery system with reconfigurable cell topology operated as a fast charging station for electric vehicles.
}\newblock {\em \DIFadd{Energies}}\DIFadd{, 14(9):2414, 2021.
}

\bibitem{lawsonSoftwareConfigurableBattery2012}
\DIFadd{Barrie Lawson.
}\newblock \DIFadd{A }{{\DIFadd{Software Configurable Battery}}}\DIFadd{.
}\newblock {\em \DIFadd{EVS26 International Battery, Hybrid and Fuel Cell Electric Vehicle Symposium}}\DIFadd{, 2012.
}

\bibitem{he2014reconfiguration}
\DIFadd{Liang He, Linghe Kong, Siyu Lin, Shaodong Ying, Yu~Gu, Tian He, and Cong Liu.
}\newblock \DIFadd{Reconfiguration-assisted charging in large-scale lithium-ion battery systems.
}\newblock \DIFadd{In }{\em \DIFadd{2014 ACM/IEEE International Conference on Cyber-Physical Systems (ICCPS)}}\DIFadd{, pages 60--71. IEEE, 2014.
}

\bibitem{kim2009dynamic}
\DIFadd{Hahnsang Kim and Kang~G Shin.
}\newblock \DIFadd{On dynamic reconfiguration of a large-scale battery system.
}\newblock \DIFadd{In }{\em \DIFadd{2009 15th IEEE Real-Time and Embedded Technology and Applications Symposium}}\DIFadd{, pages 87--96. IEEE, 2009.
}

\bibitem{han2021analysis}
\DIFadd{Weiji Han, Anton Kersten, Changfu Zou, Torsten Wik, Xiaoliang Huang, and Guangzhong Dong.
}\newblock \DIFadd{Analysis and estimation of the maximum switch current during battery system reconfiguration.
}\newblock {\em \DIFadd{IEEE Transactions on Industrial Electronics}}\DIFadd{, 69(6):5931--5941, 2021.
}

\bibitem{chenSneakCircuitTheory2021}
\DIFadd{Si-Zhe Chen, Yule Wang, Guidong Zhang, Le~Chang, and Yun Zhang.
}\newblock \DIFadd{Sneak }{{\DIFadd{Circuit Theory Based Approach}}} \DIFadd{to }{{\DIFadd{Avoiding Short-Circuit Paths}}} \DIFadd{in }{{\DIFadd{Reconfigurable Battery Systems}}}\DIFadd{.
}\newblock {\em \DIFadd{IEEE Transactions on Industrial Electronics}}\DIFadd{, 68(12):12353--12363, 2021.
}

\bibitem{heExploringAdaptiveReconfiguration2013}
\DIFadd{Liang He, Lipeng Gu, Linghe Kong, Yu~Gu, Cong Liu, and Tian He.
}\newblock \DIFadd{Exploring }{{\DIFadd{Adaptive Reconfiguration}}} \DIFadd{to }{{\DIFadd{Optimize Energy Efficiency}}} \DIFadd{in }{{\DIFadd{Large-Scale Battery Systems}}}\DIFadd{.
}\newblock \DIFadd{In }{\em \DIFadd{2013 }{{\DIFadd{IEEE}}} \DIFadd{34th }{{\DIFadd{Real-Time Systems Symposium}}}}\DIFadd{, pages 118--127, December 2013.
}

\bibitem{hongwenheStateofChargeEstimationLithiumIon2011}
\DIFadd{Hongwen He, Rui Xiong, Xiaowei Zhang, Fengchun Sun, and JinXin Fan.
}\newblock \DIFadd{State-of-}{{\DIFadd{Charge Estimation}}} \DIFadd{of the }{{\DIFadd{Lithium-Ion Battery Using}}} \DIFadd{an }{{\DIFadd{Adaptive Extended Kalman Filter Based}}} \DIFadd{on an }{{\DIFadd{Improved Thevenin Model}}}\DIFadd{.
}\newblock {\em \DIFadd{IEEE Transactions on Vehicular Technology}}\DIFadd{, 60(4):1461--1469, May 2011.
}

\bibitem{mousavig.VariousBatteryModels2014}
\DIFadd{S.M. Mousavi~G. and M.~Nikdel.
}\newblock \DIFadd{Various battery models for various simulation studies and applications.
}\newblock {\em \DIFadd{Renewable and Sustainable Energy Reviews}}\DIFadd{, 32:477--485, April 2014.
}

\bibitem{ciNovelDesignAdaptive2007}
\DIFadd{Song Ci, Jiucai Zhang, Hamid Sharif, and Mahmoud Alahmad.
}\newblock \DIFadd{A }{{\DIFadd{Novel Design}}} \DIFadd{of }{{\DIFadd{Adaptive Reconfigurable Multicell Battery}}} \DIFadd{for }{{\DIFadd{Power-Aware Embedded Networked Sensing Systems}}}\DIFadd{.
}\newblock \DIFadd{In }{\em {{\DIFadd{IEEE GLOBECOM}}} \DIFadd{2007-2007 }{{\DIFadd{IEEE Global Telecommunications Conference}}}}\DIFadd{, pages 1043--1047, November 2007.
}

\bibitem{alahmadBatterySwitchArray2008}
\DIFadd{Mahmoud Alahmad, Herb Hess, Mohammad Mojarradi, William West, and Jay Whitacre.
}\newblock \DIFadd{Battery switch array system with application for }{{\DIFadd{JPL}}}\DIFadd{'s rechargeable micro-scale batteries.
}\newblock {\em \DIFadd{Journal of Power Sources}}\DIFadd{, 177(2):566--578, March 2008.
}

\bibitem{kimDependableEfficientScalable2010b}
\DIFadd{Hahnsang Kim and Kang~G. Shin.
}\newblock \DIFadd{Dependable, efficient, scalable architecture for management of large-scale batteries.
}\newblock \DIFadd{In }{\em \DIFadd{Proceedings of the 1st }{{\DIFadd{ACM}}}\DIFadd{/}{{\DIFadd{IEEE International Conference}}} \DIFadd{on }{{\DIFadd{Cyber-Physical Systems}}}}\DIFadd{, }{{\DIFadd{ICCPS}}} \DIFadd{'10, pages 178--187, }{\DIFadd{New York, NY, USA}}\DIFadd{, April 2010. }{\DIFadd{Association for Computing Machinery}}\DIFadd{.
}

\bibitem{kimBalancedReconfigurationStorage2011a}
\DIFadd{Younghyun Kim, Sangyoung Park, Yanzhi Wang, Qing Xie, Naehyuck Chang, Massimo Poncino, and Massoud Pedram.
}\newblock \DIFadd{Balanced reconfiguration of storage banks in a hybrid electrical energy storage system.
}\newblock \DIFadd{In }{\em \DIFadd{2011 }{{\DIFadd{IEEE}}}\DIFadd{/}{{\DIFadd{ACM International Conference}}} \DIFadd{on }{{\DIFadd{Computer-Aided Design}}} \DIFadd{(}{{\DIFadd{ICCAD}}}\DIFadd{)}}\DIFadd{, pages 624--631, November 2011.
}

\bibitem{taesickimSeriesconnectedSelfreconfigurableMulticell2012a}
\DIFadd{Taesic Kim, Wei Qiao, and Liyan Qu.
}\newblock \DIFadd{A series-connected self-reconfigurable multicell battery capable of safe and effective charging/discharging and balancing operations.
}\newblock \DIFadd{In }{\em \DIFadd{2012 }{{\DIFadd{Twenty-Seventh Annual IEEE Applied Power Electronics Conference}}} \DIFadd{and }{{\DIFadd{Exposition}}} \DIFadd{(}{{\DIFadd{APEC}}}\DIFadd{)}}\DIFadd{, pages 2259--2264, February 2012.
}

\bibitem{6843711}
\DIFadd{Liang He, Linghe Kong, Siyu Lin, Shaodong Ying, Yu~Gu, Tian He, and Cong Liu.
}\newblock \DIFadd{Reconfiguration-assisted charging in large-scale }{{\DIFadd{Lithium-ion}}} \DIFadd{battery systems.
}\newblock \DIFadd{In }{\em \DIFadd{2014 }{{\DIFadd{ACM}}}\DIFadd{/}{{\DIFadd{IEEE International Conference}}} \DIFadd{on }{{\DIFadd{Cyber-Physical Systems}}} \DIFadd{(}{{\DIFadd{ICCPS}}}\DIFadd{)}}\DIFadd{, pages 60--71, April 2014.
}
